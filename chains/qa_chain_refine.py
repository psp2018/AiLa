...
def build_qa_chain():
    ...
    if os.path.exists(index_path):
        ...
        docs_gdpr = load_and_split_documents("data/gdpr_articles.json", source_label="GDPR")
        docs_fadp = load_and_split_documents("data/swiss_fadp_articles.json", source_label="FADP")
...
Refined answer:"""
    ...
    def is_gdpr_query(query):
        keywords = [
            "article", "gdpr", "fadp", "data protection", "privacy", "personal data",
            "controller", "processor", "data subject", "rights", "law", "regulation", "data minimisation",
            "data minimization", "jurisdictions"
        ]
    ...
    def invoke(query, regulation_override=None):
        ...
        if target_article:
            exact_match = [
                ...
                if doc.metadata.get("article", "").strip().lower() == target_article.strip().lower()
            ]
            if exact_match:
